...
contract("Reputation Updates", accounts => {
  ...
  describe("when added", () => {
    ...
    it("should set the correct domain and skill reputation change amount in log for payments", async () => {
      ...
      await metaColony.addPayment(1, MAX, RECIPIENT, clnyToken.address, WAD, 1, GLOBAL_SKILL_ID);
      const paymentId = await metaColony.getPaymentCount();

      const payment = await metaColony.getPayment(paymentId);
      await metaColony.moveFundsBetweenPots(1, MAX, MAX, 1, payment.fundingPotId, WAD.add(WAD.divn(10)), clnyToken.address);
      await metaColony.finalizePayment(1, MAX, paymentId);
    ...
    it("should not add entries to the reputation log for payments that are not in the colony home token", async () => {
      ...
      await metaColony.addPayment(1, MAX, RECIPIENT, otherToken.address, WAD, 1, GLOBAL_SKILL_ID);
      const paymentId = await metaColony.getPaymentCount();

      const payment = await metaColony.getPayment(paymentId);
      await metaColony.moveFundsBetweenPots(1, MAX, MAX, 1, payment.fundingPotId, WAD.add(WAD.divn(10)), otherToken.address);
      await metaColony.finalizePayment(1, MAX, paymentId);
