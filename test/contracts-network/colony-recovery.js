...
import { MAX, SPECIFICATION_HASH } from "../../helpers/constants";
...
contract("Colony Recovery", accounts => {
  ...
  describe("when using recovery mode", () => {
    ...
    it("should not be able to add and remove roles when in recovery", async () => {
      await colony.enterRecoveryMode();
      await checkErrorRevert(colony.setAdministrationRole(1, MAX, accounts[1], 1, true), "colony-in-recovery-mode");
      await checkErrorRevert(colony.setAdministrationRole(1, MAX, accounts[1], 1, false), "colony-in-recovery-mode");
