...
contract("ColonyTask", accounts => {
  ...
  describe("when creating tasks", () => {
    ...
    it("should fail if a non-admin user tries to make a task", async () => {
      ...
      await checkErrorRevert(colony.makeTask(1, MAX, SPECIFICATION_HASH, 1, 1, 0, { from: OTHER }), "ds-auth-unauthorized");
    ...
    it("should log TaskAdded and FundingPotAdded events", async () => {
      await expectAllEvents(colony.makeTask(1, MAX, SPECIFICATION_HASH, 1, 3, 0), ["TaskAdded", "FundingPotAdded"]);
    });
  ...
  describe("when updating tasks", () => {
    ...
    it("should allow to change manager role if the user agrees", async () => {
      ...
      await executeSignedRoleAssignment({
        ...
        sigTypes: [0, 0],
        args: [taskId, COLONY_ADMIN, 1, MAX]
      });
  ...
  describe("when cancelling a task", () => {
    ...
    it("should be possible to return funds back to the domain", async () => {
      ...
      await colony.moveFundsBetweenPots(1, MAX, MAX, 1, taskPotId, 100, ethers.constants.AddressZero);

      // And another token
      await otherToken.mint(colony.address, 101);
      await colony.claimColonyFunds(otherToken.address);
      await colony.moveFundsBetweenPots(1, MAX, MAX, 1, taskPotId, 100, otherToken.address);
      ...
      await colony.moveFundsBetweenPots(1, MAX, MAX, taskPotId, domain.fundingPotId, originalTaskEtherBalance, ethers.constants.AddressZero);
      await colony.moveFundsBetweenPots(1, MAX, MAX, taskPotId, domain.fundingPotId, originalTaskTokenBalance, token.address);
      await colony.moveFundsBetweenPots(1, MAX, MAX, taskPotId, domain.fundingPotId, originalTaskOtherTokenBalance, otherToken.address);
