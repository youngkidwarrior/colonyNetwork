...
import { MAX, UINT256_MAX, INT128_MAX, WAD, SECONDS_PER_DAY, MAX_PAYOUT, GLOBAL_SKILL_ID } from "../../helpers/constants";
...
contract("Colony Expenditure", accounts => {
  ...
  before(async () => {
    ...
    await colony.setAdministrationRole(1, MAX, ADMIN, 1, true);
    await colony.setArbitrationRole(1, MAX, ARBITRATOR, 1, true);
  ...
  describe("when adding expenditures", () => {
    it("should allow admins to add expenditure", async () => {
      ...
      await colony.makeExpenditure(1, MAX, 1, { from: ADMIN });
    ...
    it("should not allow non-admins to add expenditure", async () => {
      await checkErrorRevert(colony.makeExpenditure(1, MAX, 1, { from: USER }), "ds-auth-unauthorized");
    });
    ...
    it("should allow arbitration users to transfer expenditures", async () => {
      ...
      await checkErrorRevert(colony.methods[transferSig](1, MAX, expenditureId, USER, { from: ADMIN }), "ds-auth-unauthorized");
      await colony.methods[transferSig](1, MAX, expenditureId, USER, { from: ARBITRATOR });
  ...
  describe("when updating expenditures", () => {
    ...
    it("should allow arbitration users to set the payoutModifier", async () => {
      await checkErrorRevert(colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, WAD.divn(2), { from: ADMIN }), "ds-auth-unauthorized");

      await colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, WAD.divn(2));
    ...
    it("should not allow arbitration users to set the payoutModifier above the maximum", async () => {
      await colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, MAX_PAYOUT_MODIFIER);

      await checkErrorRevert(
        colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, MAX_PAYOUT_MODIFIER.addn(1)),
    ...
    it("should not allow arbitration users to set the payoutModifier below the minimum", async () => {
      await colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, MIN_PAYOUT_MODIFIER);

      await checkErrorRevert(
        colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, MIN_PAYOUT_MODIFIER.subn(1)),
        "colony-expenditure-payout-modifier-too-small"
      );
    });

    it("should allow arbitration users to set the claimDelay", async () => {
      await checkErrorRevert(colony.setExpenditureClaimDelay(1, MAX, expenditureId, SLOT0, SECONDS_PER_DAY, { from: ADMIN }), "ds-auth-unauthorized");

      await colony.setExpenditureClaimDelay(1, MAX, expenditureId, SLOT0, SECONDS_PER_DAY);
  ...
  describe("when finalizing expenditures", () => {
    ...
    it("cannot finalize expenditure if it is not fully funded", async () => {
      ...
      const expenditure = await colony.getExpenditure(expenditureId);
      await colony.moveFundsBetweenPots(1, MAX, MAX, domain1.fundingPotId, expenditure.fundingPotId, WAD, token.address);
  ...
  describe("when claiming expenditures", () => {
    ...
    it("should allow anyone to claim on behalf of the slot, in multiple tokens", async () => {
      ...
      const expenditure = await colony.getExpenditure(expenditureId);
      await colony.moveFundsBetweenPots(1, MAX, MAX, domain1.fundingPotId, expenditure.fundingPotId, WAD, token.address);
      await colony.moveFundsBetweenPots(1, MAX, MAX, domain1.fundingPotId, expenditure.fundingPotId, WAD, otherToken.address);
      await colony.finalizeExpenditure(expenditureId, { from: ADMIN });
    ...
    it("should automatically reclaim funds for payoutModifiers of -1", async () => {
      await colony.setExpenditureRecipient(expenditureId, SLOT0, RECIPIENT, { from: ADMIN });
      await colony.setExpenditurePayout(expenditureId, SLOT0, token.address, WAD, { from: ADMIN });
      await colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, WAD.neg());
    ...
    it("should automatically reclaim funds for payoutModifiers between -1 and 0", async () => {
      await colony.setExpenditureRecipient(expenditureId, SLOT0, RECIPIENT, { from: ADMIN });
      await colony.setExpenditurePayout(expenditureId, SLOT0, token.address, WAD, { from: ADMIN });
      await colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, WAD.divn(3).neg()); // 2/3 payout
    ...
    it("should scale down payout by payoutScalar", async () => {
      ...
      await colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, WAD.divn(2).neg());
    ...
    it("should scale up payout by payoutScalar", async () => {
      ...
      await colony.setExpenditurePayoutModifier(1, MAX, expenditureId, SLOT0, WAD);
    ...
    it("should not overflow when using the maximum payout * modifier", async () => {
      ...
      const expenditure = await colony.getExpenditure(expenditureId);
      await colony.moveFundsBetweenPots(1, MAX, MAX, domain1.fundingPotId, expenditure.fundingPotId, MAX_PAYOUT, token.address);
      await colony.finalizeExpenditure(expenditureId, { from: ADMIN });
      await colony.claimExpenditurePayout(expenditureId, SLOT0, token.address);
  ...
  describe("when cancelling expenditures", () => {
    ...
    it("should let funds be reclaimed", async () => {
      ...
      await checkErrorRevert(
        colony.moveFundsBetweenPots(1, MAX, MAX, expenditure.fundingPotId, domain1.fundingPotId, WAD, token.address),
        "colony-funding-expenditure-bad-state"
      );

      await colony.cancelExpenditure(expenditureId, { from: ADMIN });
      await colony.moveFundsBetweenPots(1, MAX, MAX, expenditure.fundingPotId, domain1.fundingPotId, WAD, token.address);
    });
  });
});
