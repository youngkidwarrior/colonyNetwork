...
contract("ColonyPermissions", accounts => {
  ...
  describe("when managing domain-level permissions", () => {
    ...
    it("should let users query for roles in domain and subdomains", async () => {
      ...
      expect(administrationRole).to.be.true;
      administrationRole = await colony.hasInheritedUserRole(FOUNDER, 1, ADMINISTRATION_ROLE, MAX, 1);
      expect(administrationRole).to.be.true;
      ...
      administrationRole = await colony.hasInheritedUserRole(USER1, 1, ADMINISTRATION_ROLE, MAX, 1);
      expect(administrationRole).to.be.false;
    ...
    it("should allow users with funding permission manipulate funds in their domains only", async () => {
      ...
      await colony.moveFundsBetweenPots(1, MAX, 0, domain1.fundingPotId, domain2.fundingPotId, WAD, token.address);
      ...
      await checkErrorRevert(
        colony.moveFundsBetweenPots(1, MAX, 0, domain1.fundingPotId, domain2.fundingPotId, WAD, token.address, { from: USER1 }),
        "ds-auth-unauthorized"
      );

      const taskId = await makeTask({ colonyNetwork, colony, domainId: 2 });
      const task = await colony.getTask(taskId);
      await colony.moveFundsBetweenPots(2, MAX, MAX, domain2.fundingPotId, task.fundingPotId, WAD, token.address, { from: USER1 });
    });
    ...
    it("should allow users with administration permission manipulate expenditures in their domains only", async () => {
      ...
      await colony.makeExpenditure(1, MAX, 1, { from: FOUNDER });
      ...
      await checkErrorRevert(colony.makeExpenditure(1, MAX, 1, { from: USER1 }), "ds-auth-unauthorized");
      await colony.makeExpenditure(2, MAX, 2, { from: USER1 });
    });
    ...
    it("should allow users with administration permission manipulate tasks/payments in their domains only", async () => {
      ...
      await colony.makeTask(1, MAX, SPECIFICATION_HASH, 1, 0, 0, { from: FOUNDER });
      ...
      await checkErrorRevert(colony.makeTask(1, MAX, SPECIFICATION_HASH, 1, 0, 0, { from: USER1 }), "ds-auth-unauthorized");

      const { logs } = await colony.makeTask(2, MAX, SPECIFICATION_HASH, 2, 0, 0, { from: USER1 });
    ...
    it("should allow users with architecture permission manipulate the structure of their subdomains only", async () => {
      ...
      await checkErrorRevert(colony.addDomain(1, MAX, 1, { from: USER1 }), "ds-auth-unauthorized");

      // Note: cannot add subdomains currently, this is just checking that the auth passed.
      await checkErrorRevert(colony.addDomain(2, MAX, 2, { from: USER1 }), "colony-parent-domain-not-root");

      // Now User1 can manipulate domain 1 subdomains
      await colony.setArchitectureRole(1, MAX, USER1, 1, true);
      hasRole = await colony.hasUserRole(USER1, 1, ARCHITECTURE_ROLE);
      expect(hasRole).to.be.true;

      // Create subdomain...
      await colony.addDomain(1, MAX, 1, { from: USER1 });
      ...
      await checkErrorRevert(colony.setAdministrationRole(1, MAX, USER2, 1, true, { from: USER1 }), "ds-auth-only-authorized-in-child-domain");
      ...
      await colony.setAdministrationRole(1, MAX, USER2, 1, true, { from: USER1 });
    });
    ...
    it("should allow users with root permission manipulate root domain permissions and colony-wide parameters", async () => {
      ...
      await colony.setArbitrationRole(1, MAX, USER2, 1, true, { from: USER1 });
      await colony.setArchitectureRole(1, MAX, USER2, 1, true, { from: USER1 });
      await colony.setFundingRole(1, MAX, USER2, 1, true, { from: USER1 });
      await colony.setAdministrationRole(1, MAX, USER2, 1, true, { from: USER1 });
    ...
    it("should allow permissions to propagate to subdomains", async () => {
      ...
      await colony.setFundingRole(1, MAX, USER2, 1, true);

      await fundColonyWithTokens(colony, token, INITIAL_FUNDING);
      // Test we can move funds between domain 1 and 2, and also 2 and 3
      await colony.moveFundsBetweenPots(1, MAX, 0, domain1.fundingPotId, domain2.fundingPotId, WAD, token.address, { from: USER2 });
    ...
    it("should not allow operations on nonexistent domains", async () => {
      ...
      await colony.makeTask(1, MAX, SPECIFICATION_HASH, 1, 0, 0);
