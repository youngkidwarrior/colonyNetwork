...
contract IReputationMiningCycle is ReputationMiningCycleDataTypes {
  ...
  /// @param jhIntermediateValue The contents of the Justification Tree at the key given by `targetKey` (see function description). The value of `targetKey` is computed locally to establish what to submit to this function.
  /// @param branchMask The branchMask of the Merkle proof that `jhIntermediateValue` is the value at key `targetKey`
  /// @param siblings The siblings of the Merkle proof that `jhIntermediateValue` is the value at key `targetKey`
  ...
  /// @param branchMask1 The branchmask for the Merkle proof that the currently accepted reputation state (given by `ColonyNetwork.getReputationRootHash()` + `ColonyNetwork.getReputationRootHashNLeaves()`, where `+` is concatenation) is at key 0x000..000 in the submitted JRH
